...
\item Show that the statistic $T(X)$ in part (a) is not complete. \\
  ...
  \begin{Solution}
    ...
    \begin{claim}
      For $i = 1, \dots, n$, $EX_i = \pi^{-1/2} + \theta$.
    \end{claim}
    \begin{claimproof}
      By a simple $u$-substitution,
      \[ E(X_i - \theta) = \int_{-\infty}^{\theta}(x - \theta) 2\pi^{-1/2}\exp\left\{ -(x -\theta^2) \right\}dm(x) = -\pi^{-1/2}\exp\left\{
          -(x-\theta)^2
    ...
    \begin{claim}
      $E[\max_i X_i] = \theta - K$ for some constant $K > 0$.
    \end{claim}
    \begin{claimproof}
      ...
      \[
        ...
        of }\t}  = \theta - K.
      \]
      Hence $E[\max_i X_i] = \theta - K$ where $K := \int_{-\infty}^{\theta}\left[ F_{\theta}(x) \right]^{n}dm(x)$.
    \end{claimproof}

    Now take $h(T(\bm{x})) := \frac{1}{n}\left( \sum_{i=1}^{n}x_i \right) - \pi^{-1/2} - K - \max_i x_i = \bar{x} - \pi^{-1/2} - K - \max_i x_i$.
    Then by claims 1 and 2,
    \[
      E_{\theta}h\circ T(X) = E_{\theta}\bar{X} - \pi^{-1/2} - K - E\left[ \max_i X_i \right] = \pi^{-1/2} + \theta - \pi^{-1/2} - K - (\theta - K) =
      0.
    \]
...
\begin{Solution}
  ...
  $R(1,\delta) = 0$. But $R(1,\delta) \geq \|a - p\|p^{2} = \|a - 1\| > 0$. This is a contradiction once again.
\end{Solution}
