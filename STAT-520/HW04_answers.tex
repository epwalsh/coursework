...
\begin{enumerate}
  \item We define the random variables $Y_{1}, \dots, Y_{75}$ where $Y_{i}$ is associated with the nitrate concentration in mg/L of the $i^{th}$ well.
    ...
    Since none all of the confidence intervals for the pairwise differences in the estimated ``rate'' parameters contain 0, there is not sufficient
    ...
    distribution function is in Figure \ref{combined_density}. Using this estimated common distribution with $\hat{\alpha} = \hat{\phi} = 3.0311$ and
