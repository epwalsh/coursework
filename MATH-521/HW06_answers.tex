...
\begin{Lemma}[Probability Integral Transformation]
  If $F$ is a cdf and $F^{-1}(x) := \sup \{ t : F(t) < x \}$, $x \in [0, 1]$, then the random variable $Z := F^{-1}(x)$ on $[0, 1]$ has distribution function $F$.
\end{Lemma}
\begin{Proof}
...
\begin{Solution}
  Let $F_n$ be the CDF of $X_n$, for each $n \geq 1$. Let $(\Omega, \mathcal{F}, P)$ be the Borel space on $[0, 1]$, and define
  \begin{align*}
    Y_n(\omega) & := \sup\{ t : F(t) < \omega \},  \ \ n \geq 1 \\
    Y(\omega) & := \sup\{ t : F(t) < \omega \}.
  \end{align*}

  \begin{claim}
    $Y$ has the same distribution as $X$, and $Y_n$ has the same distribution as $X_n$ for each $n \geq 1$.
  \end{claim}
  \begin{claimproof}
    This follows from Lemma 1 (the PIT).
  \end{claimproof}

  \begin{claim}
    $Y_n \rightarrow Y$ almost surely.
  \end{claim}
  \begin{claimproof}
