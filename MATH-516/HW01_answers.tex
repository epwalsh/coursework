...
\[ f(t) := \mu(A \cap [0,t]) \]
for all $t \in [0,1]$. Clearly $f$ is monotone increasing and $r > f(0) = \mu(\emptyset) = 0$ while $r < f(1) = \mu(A)$. By question 4, $f$ is
...
Let $\mathcal{S}$ denote the $\sigma$-algebra of all Legesgue measurable subsets of $[0,1]$. Let $m$ denote the Lebesgue measure, and let $\mu :
...
\begin{claim}
$F$ is differentiable with a constant derivative of 1.
\end{claim}
\begin{claimproof}
