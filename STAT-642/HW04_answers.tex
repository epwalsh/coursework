...
So $\lim_{k\rightarrow\infty}E|X_{n_{k}}-X|^{r} \geq \delta\cdot \epsilon^{r}$. This is a contradiction.
\end{Proof}
...
\[ \left|\frac{\sum_{j=0}^{n-1}X_{j}}{b_{n}}\right| \leq \left| \frac{\sum_{j=0}^{n-1}X_{j}}{b_{n-1}}\right|\times M  = \left|
