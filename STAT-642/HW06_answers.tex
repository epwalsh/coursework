...
Then clearly $F(t_{0}) \geq p_{0}$ if and only if 
\[ \inf\left\{ x \in \mathbb{R} : F(x) \geq p_{0} \right\} \leq t_{0}\] 
if and only if $F^{-1}(p_{0}) \leq t_{0}$.
\end{Proof}
...
\end{tcolorbox}
\begin{Proof}
By the result in question 3, $C(F)$ and $C(G)$ are countable. So $C(F) \cup C(G)$ is countable. Thus, $D := [C(F) \cup C(G)]^{c}$ is dense in
$\mathbb{R}$ and $F_{n}(x) \rightarrow F(x)$, $F_{n}(x) \rightarrow G(x)$ for every $x \in D$. So $F \equiv G$ on $D$. By the result in question 4, $F
\equiv G$.
\end{Proof}
